...
class BrainAnalytics:
	def __init__(self, memory_file="solitaire_parallel_brain.pkl"):
		...
		if not os.path.exists(memory_file):
			raise FileNotFoundError(f"Brain file '{memory_file}' not found. Run main.py first.")
		...
		# Calculate turns based on population count (32 - pegs)
	...
	def generate_timeline_maps(self):
		...
		for r, c in self.r_c_to_bit:
			...
			# Metric 1: Earliest turn state changed from initial
			...
			# Metric 2: Latest turn peg was active
		...
		# Awakening Map
		axes[0].imshow(first_change_map, cmap='coolwarm', vmin=0, vmax=20)
		axes[0].set_title("The Awakening: Earliest Turn of Change\n(Blue = Immediate Action)", fontsize=12,
		                  fontweight='bold')
		axes[0].axis('off')

		# Last Stand Map
		axes[1].imshow(last_active_map, cmap='turbo', vmin=10, vmax=32)
		...
		for ax, data in zip(axes, [first_change_map, last_active_map]):
			for r, c in self.r_c_to_bit:
				...
				ax.text(c, r, f"T{int(val)}", ha="center", va="center", color=color, fontsize=9, fontweight='bold')
	...
	def generate_expected_flux(self):
		...
		# Flux = Absolute difference in probability between turns
		...
		plt.title('Time Center of Gravity: When does the action happen?', fontsize=14, fontweight='bold')
		plt.axis('off')
