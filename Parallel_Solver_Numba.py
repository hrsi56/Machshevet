...
# 2. Optimized Parallel Expansion (Buffer Reuse Optimization) ⭐
# Accepts 'out_buffer' externally to avoid repeated memory allocation overhead.
@njit(parallel=True, nogil=True)
def expand_batch_parallel(boards, table, moves_mask, checks_src, checks_dst, out_buffer):
    num_boards = len(boards)
    num_moves = len(moves_mask)

    # Note: No dynamic memory allocation here.
    # We write directly into the pre-allocated memory block.
    ...
    for i in prange(num_boards):
       ...
       for m in range(num_moves):
          ...
          if (curr & checks_src[m] == checks_src[m]) and (curr & checks_dst[m] == 0):
             ...
             # Compute canonical form and store in the shared buffer
...
class PegSolitaireSolver:
    ...
    def __init__(self):
       ...
       # Board Initialization and Mapping
       idx = 0
       for r in range(7):
          for c in range(7):
    ...
    def train(self):
       ...
       # --- Optimization: One-time Memory Allocation (Buffer Reuse) ---
       # Calculate the maximum possible size for the results buffer.
       num_possible_moves = len(self.moves_mask)
       buffer_size = self.BATCH_SIZE * num_possible_moves

       # This shared buffer is recycled in every iteration of the loop.
       ...
       while queue:
          ...
          # 2. Parallel Processing (Using Shared Buffer)
          # Pass the buffer to the function, which writes results directly into it.
          expand_batch_parallel(
             ...
             self.moves_check_src,
             self.moves_check_dst,
             shared_buffer  # <-- Pass the pre-allocated buffer
          )

          # 3. Filter & Update (Slicing)
          # We must read only the relevant section of the buffer!
          relevant_size = current_batch_size * num_possible_moves
          valid_results_view = shared_buffer[:relevant_size]

          # Perform np.unique only on the relevant view
          unique_candidates = np.unique(valid_results_view)

          # Clean the buffer for the next iteration (Optimization)
